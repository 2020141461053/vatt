...
class MOSI(loading.BaseDMVRFactory, abc.ABC):
  """Base class for a toy factory."""

  _BASE_DIR = 'data/dataset/mosi_shards'
  #_NUM_CLASSES = 51
  ...
  def __init__(self, subset = 'train', split = 1):
    ...
    del split
    if subset not in MOSI._TABLES:
      raise ValueError('Invalid subset "{}". The available subsets are: {}'
                       .format(subset, MOSI._TABLES.keys()))

    super().__init__(MOSI._BASE_DIR,
                     table=f'{subset}',
                     source='tfrecord',
  ...
  def _build(self,
    ...
    processing.add_audio(
         ...
         preprocessor_builder=self.preprocessor_builder,
         postprocessor_builder=self.postprocessor_builder,
         input_feature_name='waveform/floats',#'WAVEFORM/features/floats'
    ...
    processing.add_label(
        ...
        decoder_builder=self.decoder_builder,
        preprocessor_builder=self.preprocessor_builder,
        input_label_index_feature_name='clip/label/index',#clip/label/index
        input_label_name_feature_name='clip/label/text',#clip/label/text
